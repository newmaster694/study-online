package study.online.search.service.impl;
...
public class CourseSearchServiceImpl implements ICourseSearchService {
	...
	public SearchPageResultDto<CourseIndex> queryCoursePubIndex(PageParams pageParams, SearchCourseParamDto courseSearchParam) {
		...
		//获取聚合结果
		List<String> mtList = getAggregation((InternalAggregations) searchResponse.getAggregations(), "mtAgg");
		List<String> stList = getAggregation((InternalAggregations) searchResponse.getAggregations(), "stAgg");
	...
	private void buildAggregation(SearchRequest request) {
		request.source().aggregation(AggregationBuilders.terms("mtAgg").field("mtName").size(100));
		request.source().aggregation(AggregationBuilders.terms("stAgg").field("stName").size(100));

	}

	private List<String> getAggregation(InternalAggregations aggregations, String aggName) {
		// 4.1.根据聚合名称获取聚合结果
